...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const featuredDev = devs.find(dev => dev.id === featuredDevId);
    if (featuredDev) return featuredDev.name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredDevId ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          devs.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeaturedDevId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
